...
object asdd1 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    var weather=rdd.map(line=>line.split("\t")).map(row=>try {weath1(getyear(row(0)),row(1).toFloat,row(2).toFloat,row(3).toFloat,row(4).toFloat,
    ...
    try {
      sparkSession.sql("select DateTime,TdegC from weather_dataset").groupBy("DateTime").max().show()
    }
  }
}
