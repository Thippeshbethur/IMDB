...
object asdd3 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    var weather=rdd.map(line=>line.split("\t")).map(row=>try {weath2(getyear(row(0)),
    ...
    try {
      sparkSession.sql("select DateTime,rh1 from weather_dataset where rh1>96").distinct().show()
    }
  }
}
case class weath2(DateTime:String,pmbar:Float,TdegC:Float,TpotK:Float,TdewdegC:Float,
                  rh1:Float,VPmaxmbr:Float,VPactmbar:Float,VPdefmbar:Float,
